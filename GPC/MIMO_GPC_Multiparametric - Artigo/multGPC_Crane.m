...
%         E = zeros(2*m*Nu,2*n*N2+m*Nu);
        E = zeros(2*m*Nur,n*(N2+N2r)+m*Nur);
    end
    if (restricaoU)
        Restr = [Restr; RU];
        c = [c; cU];
%         E =[E; zeros(m*Nu,n*N2) -eye(m*Nu,m*Nu) zeros(m*Nu,n*N2);zeros(m*Nu,n*N2) eye(m*Nu,m*Nu) zeros(m*Nu,n*N2)];
        E =[E; zeros(m*Nur,n*N2) -eye(m*Nur,m*Nur) zeros(m*Nur,n*N2r);zeros(m*Nur,n*N2) eye(m*Nur,m*Nur) zeros(m*Nur,n*N2r)];
...
%         E = [E; zeros(n*N2,n*N2+m*Nu) -RestrY;zeros(n*N2,n*N2+m*Nu) RestrY];
        E = [E; zeros(n*N2r,n*N2+m*Nur) -eye(n*N2r,n*N2r);zeros(n*N2r,n*N2+m*Nur) eye(n*N2r,n*N2r)];
%         E = [E; zeros(1,Nu+N2) -1;zeros(1,Nu+N2) 1]; 
    end

% if (toSave)
%     tstart = tic;
%     [Pn,Fi,Gi,details] = multparametric(H,G,R,n,m,Restr,c,E,Nu,Nur,N2,N2r);

...
load('mptoptions.mat', 'mptOptions2')
mptOptions.sdpsettings = mptOptions2.sdpsettings;
...
% G*U <= W+E*X
F = F + (Matrices.G*U <= Matrices.W + Matrices.E*X);

% bndA*x <= bndb
if ~isempty(Matrices.bndA),
    F = F + (Matrices.bndA*X <= Matrices.bndb);
end
...
for ii = 1:length(alph_vars),
    ...
    if ~isempty(Matrices.alphabet{ivar}),
        F = F + (ismember(U(ivar), Matrices.alphabet{ivar}));
    else

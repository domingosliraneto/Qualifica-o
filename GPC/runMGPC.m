...
% tic    
% [Pn,Fi,Gi,details] = multparametric(H,G,R,cnstrMatrix,Restr,c);
% toc
clear online_MPT
% load('mptResult.mat')
load('mptCraneNp46.mat') 
...
for k = 1:samples
    ...
    % Sway angle Theta
    uThsim = mountVectorU(U,nbdTh,k,0);
    yThsim = mountVectorY(YTh,naTh,k,0);
    YTh(:,k) = -AThsim*yThsim+BThd*uThsim;
...
%     du = MultconstrainedMGPC(F,G_,Y,na,U,nbd,Ref,r,w,alpha,N2,n,k,Pn,Fi,Gi);
%     telapsed = toc(tstart);
